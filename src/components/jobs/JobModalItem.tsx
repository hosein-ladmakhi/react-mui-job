...
import { TJobDetailModalContent } from "../../types/models";
...
const JobModalItem: FC = () => {
   const { modalContent: job } = useAppContext() as TJobDetailModalContent;
   ...
   return (
      <Modal subject={JOB_DETAIL_SUBJECT}>
         ...
         <Typography mt={2} variant="caption">
            <FlexBox justify="flex-start">
               <icons.MuiAttachMoneyIcon className={classes.icon} /> Salary{" "}
               {numberFormat(job?.salary)}
            </FlexBox>
         </Typography>
         ...
         <Typography variant="caption">
            <FlexBox justify="flex-start">
               <icons.MuiGlobeIcon className={classes.icon} /> {job?.country}{" "}
               {job?.provience}
            </FlexBox>
         </Typography>
